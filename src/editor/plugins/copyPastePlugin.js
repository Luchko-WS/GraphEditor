var copyPastePlugin = function(graph, mxEvent) {
    ...
    mxUtils.setOpacity(textInput, 1);
    ...
    mxEvent.addListener(textInput, 'copy', mxUtils.bind(this, copyFunc));

    // Handles cut event by removing cells putting XML into text input
    mxEvent.addListener(textInput, 'cut', mxUtils.bind(this, cutFunc));
    ...
    var pasteFunc = function(eventt) {
        ...
        if (graph.isEnabled()) {
            var xml = extractGraphModelFromEvent(eventt);
        ...
        console.dir(mxClipboard);

        textInput.select();
    }

    // Handles paste event by parsing and inserting XML
    mxEvent.addListener(textInput, 'paste', pasteFunc);
